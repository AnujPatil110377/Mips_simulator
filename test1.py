def toAsm(mc, asm):
    ...
    elif opcode in op:  # I-types and J-types
        if opcode == '000010' or opcode == '000011':  # J-types (j and jal)
            imm = mc[6:]
            asm.append(f"{op[opcode]} {int(imm, 2)}")
        else:
            rs = mc[6:11]
            rt = mc[11:16]
            imm0 = mc[16:]
...
def sim(inD, reg, memory, binList):
    ...
    while PC in inD:
        ...
        if part[0] == 'addi':
            ...
            imm = int(part[1])
            reg['$31'] = PC + 4  # Store the return address in register $31
            PC = (PC & 0xF0000000) | (imm << 2)
            counts['Jump'] += 1
...
binList = [
    ...
    '00000000001000000010000000111111',  # calc $4, $1
...
inD = {i * 4: asmList[i] for i in range(len(asmList))}

sim(inD, reg, memory, binList)